...
class FormRadiologoModel:
    ...
    def __init__(self, id_formulario_radiologo, id_paciente, id_especialista, fecha_toma_examen, institucion_prestadora, tipo_examen, resultado_mamog_tamizacion, resultado_mamog_diagnostica, resultado_mamog_mamaria, conducta_sugerida, ciudad, departamento, zona):
        self.id_formulario_radiologo = id_formulario_radiologo
    ...
    def add(formulario):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO formulario_radiologo (id_formulario_radiologo, id_paciente, id_especialista, fecha_toma_examen, institucion_prestadora, tipo_examen, resultado_mamog_tamizacion, resultado_mamog_diagnostica, resultado_ecograf_mamaria, conducta_sugerida, ciudad, departamento, zona)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id_formulario_radiologo
            """, (formulario.id_formulario_radiologo, formulario.id_paciente, formulario.id_especialista, formulario.fecha_toma_examen, formulario.institucion_prestadora, formulario.tipo_examen, formulario.resultado_mamog_tamizacion, formulario.resultado_mamog_diagnostica, formulario.resultado_mamog_mamaria, formulario.conducta_sugerida, formulario.ciudad, formulario.departamento, formulario.zona))
    ...
    def get_by_id(id):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM formulario_radiologo WHERE id_formulario_radiologo = %s", (id,))
            form_data = cursor.fetchone()
            cursor.close()
            conn.close()

            if form_data:
                return FormRadiologo(*form_data)
            else:
                return None
        except Exception as e:
            print('ERROR: al consultar la tabla formulario_radiologo',e)
